...
namespace AyaMod.Content.Prefixes.CameraPrefixes.ExtraPrefixes
{
    ...
    public class Abundant() : ExtraCameraPrefix(focusSpeedMult: 0.9f)
    {
        public override void Load()
        {
            ...
            AyaGlobalProjectile.OnProjectileSpawn += AbundantSpawn;
            AyaGlobalProjectile.OnProjectileHitNPC += OnHitHeal;
            //CameraSystem.OnPostSetupContent += Hack;
        }

        public static void AbundantSpawn(Projectile projectile, Terraria.DataStructures.IEntitySource source)
        {
            if (projectile.hostile) return;
            Player player = Main.player[projectile.owner];
            if (!player.TryGetHeldModItem(out ModItem moditem) || moditem is not BaseCamera || moditem.Item.prefix != PrefixType<Abundant>()) return;
        ...
        public static void OnHitHeal(Projectile projectile, NPC target, NPC.HitInfo info, int damageDone)
        {
